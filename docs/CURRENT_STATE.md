...
- Chat 三模式 prompt 已拆分到 `backend/prompts/chat_modes/`；请求会携带最近会话历史，索引类上下文使用稳定 `[I-xx]` 编号，原文类上下文使用稳定 `[P-xx]` 编号；`wide` 模式会返回实际纳入范围，`deep` 模式会累计已注入文献并注入原始文件文本（pdf/txt/docx 解析结果），而不是索引 Markdown。
